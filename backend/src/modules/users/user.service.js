...
const createUser = async ({ 
    ...
    try{
        ...
        const existingEmail = await client.query('SELECT * FROM sample WHERE email = $1 LIMIT 1', [email]);
        if (existingEmail.rows.length > 0) {
            throw new Error('Email already exists');
        };

        // Check if username already exist
        const existingUsername = await client.query('SELECT * FROM sample WHERE username = $1 LIMIT 1', [username]);
        if (existingUsername.rows.length > 0) {
            throw new Error('Username already exists')
        }

        // Check if phonenumber already used
        const existingPhoneNumber = await client.query('SELECT * FROM sample WHERE phone_number = $1 LIMIT 1',[phone_number]);
        ...
        const existingUser = await client.query(
            'SELECT 1 FROM sample WHERE first_name = $1 AND last_name = $2 AND date_of_birth = $3 LIMIT 1',
        ...
        const insertUser = await client.query(
            'INSERT INTO sample (username, email, phone_number, first_name, middle_name, last_name, date_of_birth, password) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
